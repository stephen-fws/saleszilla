...
def run_inactive_followup_scan(anchor_date: date | None = None) -> dict:
    ...
    # Per-run cap — keeps the daily/weekly load on agentflow predictable.
    # Excess potentials remain eligible for the next run (idempotency check
    # passes for them since no insight got created).
    INACTIVE_FU_PER_RUN_CAP = 10
